    ...
    pub fn to_provider_json_value(&self) -> Value {
        ...
        match self.app_type {
            ...
            AppType::OpenCode => {
                ...
                // 从 opencode_models 构建 models map（如果列表非空）
                // 否则回退到旧的 flat fields 方式（向后兼容模板）
                ...
                if !self.opencode_models.is_empty() {
                    // 使用 opencode_models 列表构建（覆盖已有 models）
                    models_obj.clear();
                    for draft in &self.opencode_models {
                        // 先从 extra 恢复额外字段
                        let mut model_obj = if let Some(obj) = draft.extra.as_object() {
                            obj.clone()
                        } else {
                            serde_json::Map::new()
                        };
                        let model_name = if draft.model_name.is_empty() {
                    ...
                    if let Some(model_id) = current_model_id {
                        ...
                        set_or_remove_u64(
                            limit_obj,
                            "output",
                            &self.opencode_model_output_limit.value,
                        );
